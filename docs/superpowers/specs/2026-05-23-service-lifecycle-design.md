# Service Lifecycle: Standalone Start/Stop
...
| `src/daemon/api/services.js` | Add processManager param; add 4 routes; fix `GET /` filter |
| `src/daemon/server.js` | Pass `processManager` to `createServicesRoutes` |
...
- Changes to `forge service` (named instances) command.
